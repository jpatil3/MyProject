...
df1 <- gCred %>% select(c(CHK_ACCT,HISTORY,SAV_ACCT,EMPLOYMENT,PRESENT_RESIDENT,
                          JOB,RESPONSE))
df1
df.cor = cor(df1)
corrplot(df.cor)
...
df2 <- gCred %>% select(c(DURATION,AMOUNT,INSTALL_RATE,AGE,NUM_CREDITS,NUM_DEPENDENTS))
df2
df.cor = cor(df2)
corrplot(df.cor)
